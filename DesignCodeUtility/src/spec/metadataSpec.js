...
describe("Metadata Handler", () => {
  ...
  it("should know if the widget element exists on the target system", done => {

    self.endPointTransceiver.serverSupports.returnsTrue()

    mockDiskMetadataAs(widgetElementTemplatePath, {tag: "my-widget-element"})

    self.metadata.initializeMetadata().then(() => {

      expect(self.metadata.widgetElementExistsOnTarget(elementTemplatePath)).toBeTruthy()
      done()
    })
  })
