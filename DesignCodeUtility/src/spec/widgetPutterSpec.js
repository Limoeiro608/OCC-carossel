...
describe("Widget Putter", () => {
  ...
  it("should let you put elementized widget template containing no element instances on the server", done => {

    self.utils.exists.returnsTrue()
    
        // Set up readMetadata for widget
        self.metadata.readMetadata.and.callFake(Promise.method(function (path, type) {
    ...
    // Set up read of elementInstancesMetadata.json
    self.utils.readJsonFile.returns ({ "elementInstances": [] })
  ...
  it("should let you put elementized web content v1 widget template containing no element instances on the server", done => {
    ...
    // Run the test
    self.widgetPutter.putWidgetInstanceTemplate(webContentTemplatePath).then(() => {

      expect(self.endPointTransceiver.updateWidgetSourceCode).urlKeysWere(["rep5678"])
      expect(self.endPointTransceiver.updateWidgetSourceCode).pathWas(webContentTemplatePath)
      expect(self.endPointTransceiver.updateWidgetSourceCode).fieldWas("source")
      expect(self.endPointTransceiver.updateWidgetSourceCode).etagWas("etag value")
  ...
  it("should not let you put elementized web content v2 widget template containing no element instances on the server", done => {
    ...
    self.widgetPutter.putWidgetInstanceTemplate(webContentTemplatePath).then(() => {
      ...
      expect(self.logger.warn).toHaveBeenCalledWith("emptyElementizedLayoutForWidget", {path: "widget/Web Content/instances/About Us Web Content Widget"})
      expect(self.endPointTransceiver.updateWidget).not.toHaveBeenCalled()
