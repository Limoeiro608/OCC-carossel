...
/**
 * Return true if the widget element exists on the target instance - this will work in transfer and non transfer mode.
 * @param path
 */
function widgetElementExistsOnTarget(path) {

  // Get the metadata for the local file first.
  const metadata = readMetadataFromDisk(path, constants.elementMetadataJson)

  // If it exists on the target, element should already exist in the cache.
  return cache.widgetElements.get(metadata.tag)
}
...
exports.writeMetadata = writeMetadata
exports.widgetElementExistsOnTarget = widgetElementExistsOnTarget
